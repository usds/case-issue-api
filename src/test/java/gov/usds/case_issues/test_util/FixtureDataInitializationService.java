...
public class FixtureDataInitializationService {
	...
	public AttachmentSubtype ensureAttachmentSubtype(String tag, AttachmentType forType, String urlTemplate) {
		Optional<AttachmentSubtype> check = _subtypeRepository.findByExternalId(tag);
		if (check.isPresent()) {
			AttachmentSubtype found = check.get();
			if (found.getForAttachmentType() != forType) {
				throw new IllegalArgumentException("Conflicting definitions for attachment subtype " + tag);
			}
			return found;
		}
		return _subtypeRepository.save(
			new AttachmentSubtype(tag, forType, "Subtype named " + tag, "Autogenerated subtype " + tag, urlTemplate));
	}
